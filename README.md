...
桌面端保留纸张翻页、键盘切页、缩放、拖动与放大镜交互；手机端根据屏幕宽度和触控设备特征切换为轻量章节目录，避免在小屏继续运行不必要的翻书计算。详情页采用统一的自然滚动、按需展开和返回逻辑。Sylva 的 Three.js 场景使用桌面与手机两档渲染密度、像素比和帧率，隐藏的卡片图片延迟加载；作品目录和详情页使用统一纯色背景，以保证长内容阅读的稳定性。构建前会验证本地运行时、图片、字体和页面哈希。
